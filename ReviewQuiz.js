...
import styles from './ReviewQuizStyle';
...
class ReviewQuiz extends Component {
  ...
  itemOption = (item, i) => {
    ...
    console.log(data.results.answered);
    const userAnswer = data.results.answered[itemQuestion.id].answered;

    //['7054'];
    //data.results.answered[itemQuestion.id].answered;
    ...
    return (
      <TouchableOpacity key={String(i)} style={styles.itemQuestion}>
        ...
        String(userAnswer).includes(item.value) ? (
  ...
  itemOptionMultiChoice = (item, i) => {
    ...
    return (
      <TouchableOpacity key={String(i)} style={styles.itemQuestion} disabled>
        ...
        <Text style={styles.txtItemQuestion}><RenderDataHTML
                              html={item?.title}
                              style={styles.txtItemQuestion}
                            />
        </Text>
  ...
  renderFillInBlanks = item => {
    ...
    const userAnswer = data.answered[itemQuestion.id].answered;
  ...
  render() {
    ...
    console.log(data, "check the review data")
    const userAnswer = data.answered[itemQuestion.id];
          ...
          <ScrollView style={styles.content}>
            ...
            {itemQuestion && (
              ...
              <View style={styles.viewQuestion}>
                 {console.log(itemQuestion, "item quetions")}
                <RenderDataHTML
                  html={itemQuestion?.questionid?.options[0].
                    title}
                  style={styles.txtTitleQuestion}
                />

                <RenderDataHTML html={itemQuestion?.questionid?.options[0].value} />
